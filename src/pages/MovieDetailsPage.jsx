    ...
    return (
        <Fragment>
            ...
            <div className="w-full h-[600px] max-w-[600px] -mt-[300px] mx-auto relative pb-10">
                <img src={tmdbAPI.imageOriginal(poster_path)} alt="" className='w-full object-cover h-full rounded-xl z-10'  />
            </div>
            ...
            {genres.length > 0 &&
                ...
                {genres.map((item, index) => (
                    <span className='text-primary border border-primary py-2 px-4 rounded-lg' key={item.id}>{item.name}</span>
                ))}
                </div>
            }
            <p className="text-center text-white text-2xl leading-relaxed max-w-[600px] mx-auto pb-10">{overview}</p>
            <MovieCredit />
            <MovieVideo />
            <MovieSimilar />
            <MovieReview/>
